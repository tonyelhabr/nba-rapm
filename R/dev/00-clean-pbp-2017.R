...
data2 <- nbastatR::get_games_play_by_play(game_ids = config$game_id_check)
...
data_filt_view
data_filt_view %>%
  filter((pts_cumsum1 != (pts_cumsum2 + abs(dplyr::lead(pts_diff_start, 1)))) &
           (pts_cumsum2 != (pts_cumsum1 + abs(dplyr::lead(pts_diff_start, 1)))))
data_filt_view %>% filter(pts1 > 3 | pts2 > 3)
#
# data %>%
#   filter(game_id == game_id_filt) %>%
#   group_by(game_id) %>%
...
# # `id`s are either 4, 6, or 7 characters in length, except for one `id`.
# data %>%
#   gather(dummy, id, matches("^x")) %>%
#   distinct(id) %>%
#   mutate(n_char = nchar(id)) %>%
#   count(n_char, sort = TRUE)
# A tibble: 4 x 2
# n_char     n
# <int> <int>
# 1      6   288
...
# 3      4    34
# 4     10     1
# # Note that no other `id`s match the last four digits, so it is ok to truncate this special case.
# id_long <- 1962937798

data <-
  data %>%
  # filter_at(
  #   vars(matches("^lineup[1|2]$")),
  #   any_vars(str_detect(., "[0-9]{10}"))
  # ) %>%
  mutate_at(
    vars(matches("^lineup[1|2]$")),
